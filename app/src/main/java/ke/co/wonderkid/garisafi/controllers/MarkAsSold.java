...
public class MarkAsSold {
    ...
    public MarkAsSold(Context context, String Ad_id, String user_id, int do_Action){
    ...
    public void like_this_ad(final  String ad_id){
            urlJsonArry = new Config().GARISAFI_API + "/mark_as_sold.php";
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        if(response.contentEquals("1")){
                           Toast.makeText(context, "act"+do_Action+" "+response, Toast.LENGTH_LONG).show();


                            if(do_Action==1) {

                                boolean result= databaseHelper.mark_as_sold(Integer.parseInt(ad_id), user_id);

                                if(result){
                                    Toast.makeText(context, "marked as sold", Toast.LENGTH_LONG).show();

                                }

                            }else {

                                boolean result=databaseHelper.delete_from_sold(Integer.parseInt(ad_id), user_id);
                                if(result){

                                    Toast.makeText(context, "removed from chat_list of sold cars", Toast.LENGTH_LONG).show();
